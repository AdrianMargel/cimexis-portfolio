...
function bind(data,recurse){
	if(recurse===undefined){
		//default to a full recursive binding
		recurse=true;
	}
...
function canUpdate(currId){
	for(let i=0;i<=currentUpdateLevel;i++){
		...
		if(toCompare>currentUpdateId[i]){
			return false;
		}else if(toCompare<currentUpdateId[i]){
			return true;
		}
	}
	return false;
...
function setupPaging(boundList,boundIndex){
	for(let i=0;i<boundList.length;i++){
		...
		if(item.selected===undefined){
			//if the item doesn't have a selected property add it
			item.selected = bind(i==boundIndex.data,true);
		}
		addIndexSub(item);
		addItemSub(item);
	}
...
//returns a clone of a bound object, this is not a deep clone
//using this allows the original data to be extended without changing the original
//changing the values of the original properties will affect the original though
//depth determines how many layers deep deep cloning will occur to
//TODO test this works in all edge cases
function derive(target,depth){
	depth=depth||0;
	if(Array.isArray(target)){
		...
		for(let i=0;i<target.length;i++){
			let item;
			if(depth==-1){
				item=derive(target[i],-1);
			}else if(depth>0){
				item=derive(target[i],depth-1);
			}else{
				item=target[i];
			}
			copy.add(item);
		...
		for(let prop in target){
			...
			if(type!="function"){
				let item;
				if(depth==-1){
					item=derive(sharedData.projects[i],-1);
				}else if(depth>0){
					item=derive(target[prop],depth-1);
				}else{
					item=target[prop];
				}
				copy[prop]=item;
