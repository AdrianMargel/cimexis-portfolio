...
class KoiAI{
	constructor(domainPos){
		this.domainPos=domainPos;

		this.targetPos=new Vector(domainPos);
		this.targetPos.sclVec(0.5);
	...
	randomTarget(){
		let rx=Math.random()*this.domainPos.x;
		let ry=Math.random()*this.domainPos.y;
...
fishList.push(new ArticKoi(new Vector(750,-100),"sun"));
fishList.push(new ArticKoi(new Vector(250,-100),"moon"));
let canvasSize=new Vector(500,500);

for(let i=0;i<fishList.length;i++){
	fishList[i].init();
	aiList.push(new KoiAI(canvasSize));
}
...
function displayAnimation(time){
	let wide=context.canvas.clientWidth;
	let high=context.canvas.clientHeight;
	let cSize=new Vector(wide,high);
	canvasSize.setVec(cSize);
